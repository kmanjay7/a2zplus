...
<?php if($updateKycApp=='yes' && $kycdata['kyc_status'] != 'yes'){ ?>
...
<form action="<?=ADMINURL.'ag/Update_kyc/saveupdatekyc?id='.md5($kycdata['id'])?>" method="post">
...
<div class="row"> 
 ...
 <?php if( !in_array($kycdata['kyc_status'],['pending','yes','complete']) ){?>

 <?php if( in_array($kycdata['kyc_status'],['','pending']) ){?>
 <div class="sub_head m_t-15">
...
<div class="col-md-2 col-lg-2 col-lg-2">
  ...
  <?php }else if($kycdata['kyc_status'] == 'reject'){?>
  <center><h2 style="color:red">KYC Rejected</h2>
  <p>"<?=$kycdata['comment'];?>"</p></center>
  <?php }else{?>
    <center><h2 style="color:red">Please Complete KYC </h2>
   <p>"<?=$kycdata['comment'];?>"</p></center>
  <?php }?>
