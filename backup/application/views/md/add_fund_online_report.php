 ...
 <div class="row">
  ...
  <div class="co-12 col-md-2 col-lg-2">
    ...
    <div class="co-12 col-md-2 col-lg-2">
        <?php $txnlist = [''=>'All','success'=>'Success','failed'=>'Failed','pending'=>'Pending','UPI'=>'UPI','RD'=>'Rupay Card','DC'=>'Debit Card','CC'=>'Credit Card','PPI'=>'Paytm Wallet','NB'=>'Internet Banking']; 
       ?>
    ...
    <div class="co-12 col-md-2 col-lg-2">
        ...
        <?=form_dropdown(array('name'=>'filterby','class'=>'form-control'),[''=>'All','orderid'=>'Order ID','txnid'=>'Transaction ID','ccdc'=>'DC / CC Number'],set_value('filterby',$filterby));?>
    </div>
                    ...
                    <div class="report-area-t">
                        <div class="row">
                            ...
                            <div class="col-lg-6 col-md-6 col-6 ">
                                ...
                                <div class="top-10-txt down_btn"> <button onclick="fnExcelReport();" class="export">Export</button>
...
<td><a href="<?php //echo ADMINURL.$folder.'/print_reciept?utp='.md5($value['reqid']);?>" target="_blank" >
<button id="ap1" class="update-btn w-101"> Print Receipt </button></a>
<button id="ap2" class="update-btn w-101 <?=complaint_status($value['complaint'],'cl')?> cp-<?=$value['id'];?>" <?php if($value['complaint']==''){?> onclick="complaint('<?=$value['id'];?>','onl_fund')"> <?php }else{ echo 'disabled';}?> <span id="plwait-<?=$value['id'];?>"><?=complaint_status($value['complaint'],'')?></span> </button>
</td>
</tr>
<?php $i++; endforeach; } ?>       
...
function CreateHiddenTable(ListOfMessages)
{
    ...
    TableMarkUp+='<thead><tr><th>Sr. No.</th><th>Order Id</th><th>Order date and time</th><th>Bank name</th><th>Operator</th><th>Bank Trans. Id</th><th>Amount</th><th>Charges</th><th>Status</th><th>Status on</th><th>Remarks</th></tr></thead>';
    TableMarkUp+="<tbody>";
    ...
    CreateHiddenTable(Messages);
        ...
        if (msie > 0 || !!navigator.userAgent.match(/Trident.*rv\:11\./)) {
            if (window.navigator.msSaveBlob) {
                ...
                navigator.msSaveBlob(blob, 'OnlineFundReport.xls');
            }
        } else { 
          $('#testAnchor')[0].click()
        }
...
$($("#testAnchor")[0]).click(function(){ 
      $('#testAnchor').attr('href', data_type + ', ' + encodeURIComponent(tab_text));
            $('#testAnchor').attr('download', 'OnlineFundReport.xls');
});
