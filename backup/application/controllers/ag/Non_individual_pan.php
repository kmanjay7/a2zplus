...
class Non_individual_pan extends CI_Controller{
      var $folder;
    var $pagename;
    public function __construct(){
        ...
        $this->load->library('session'); 
               ...
               $this->load->helper('security');
                agsession_check();
                $this->folder = 'ag';
                $this->pagename = 'Non_individual_pan';
        }
...
$amount = $comi_array['surcharge_fixed'];
               ...
               $data['redirect'] = base_url($this->folder.'/'.$this->pagename.'?id='.$id.'&tab='.$data['tab'] );  
                agview('non_individual_pan',$data); 
  ...
  public function saveupdate(){
    ...
    if(!empty($pdata['ao_cityarea']) && !empty($pdata['contact']) && !empty($pdata['email']) && !empty($pdata['name_on_pancard'])){
        ...
        redirect(ADMINURL.$this->folder.'/non_individual_pan_uploads?id='.md5($lastid) );
        }else{
