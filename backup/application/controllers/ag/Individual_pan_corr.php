...
class Individual_pan_corr extends CI_Controller{
      var $folder;
    var $pagename;
    public function __construct(){
        ...
        $this->load->library('session'); 
               ...
               $this->load->helper('security');
                agsession_check();
                $this->folder = 'ag';
                $this->pagename = 'Individual_pan_corr';
        }
    ...
    public function index(){ 
                ...
                $data['title'] = 'Individual Pancard Correction';
...
$amount = $comi_array['surcharge_fixed'];
               ...
               $data['redirect'] = base_url($this->folder.'/'.$this->pagename.'?id='.$id.'&tab='.$data['tab'] );  
                agview('individual_pan_corr',$data); 
  ...
  public function saveupdate(){
    ...
    if( !empty($pdata['contact']) && !empty($pdata['email']) && !empty($pdata['name_on_aadhar'])){
        ...
        redirect(ADMINURL.$this->folder.'/individual_pan_corr_uploads?id='.md5($lastid) );
        }else{
    ...
    $this->session->set_flashdata('error','all required fields are mandatory!');
    redirect(ADMINURL.$this->folder.'/'.$this->pagename );  
    }
