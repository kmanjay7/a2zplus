...
class Pan_new_uploads extends CI_Controller{
	  var $folder;
    var $pagename;
...
public function upload(){
       ...
       if( $post['filename'] == 'form49a' ){
            $documenttype = 'Form 49 A(Part-1)';
       }else if( $post['filename'] == 'form49b' ){
            $documenttype = 'Form 49 A(Part-2)';
       }else if( $post['filename'] == 'raidproof' ){
            $documenttype = 'RA Address Proof';
            $compress = true;
       }else if( $post['filename'] == 'idproofimg' ){
            $documenttype = 'Proof of Identity';
            $pnup['id_proof'] = 1;
            $compress = true;    
            $this->c_model->saveupdate('dt_pancard',$pnup,null,['id'=>$id ]); 
       }else if( $post['filename'] == 'addproofimg' ){
         $documenttype = 'Proof of Address';
         $pnup['address_proof'] = 1;  
         $compress = true;  
         $this->c_model->saveupdate('dt_pancard',$pnup,null,['id'=>$id ]);
...
redirect(ADMINURL.$this->folder.'/pan_new_uploads?id='.md5($post['id']) );  
exit;  
}
