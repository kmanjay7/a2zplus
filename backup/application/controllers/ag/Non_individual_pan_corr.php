...
class Non_individual_pan_corr extends CI_Controller{
    ...
    public function __construct(){
        ...
        $this->load->library('session'); 
               ...
               $this->load->helper('security');
                agsession_check();
                $this->folder = 'ag';
                $this->pagename = 'Non_individual_pan_corr';
        }
    ...
    public function index(){ 
                ...
                $data['title'] = 'Non Individual Pancard Correction ';
...
$amount = $comi_array['surcharge_fixed'];
               ...
               $data['applicant_type'] =  $this->input->get('ptype')?$this->input->get('ptype'):'';
               $data['name_title'] = '';
               $data['first_name'] = '';
               $data['middle_name'] = '';
               $data['last_name'] = '';
               ...
               $data['redirect'] = base_url($this->folder.'/'.$this->pagename.'?id='.$id.'&tab='.$data['tab'] );  
                agview('non_individual_pan_corr',$data); 
  ...
  public function saveupdate(){
    $comi_array = $this->c_model->getSingle('dt_set_commission',['operatorid'=>25,'serviceid'=>8],'surcharge_fixed,surcharge_percent');
    $deductAmount = $comi_array['surcharge_fixed']; 
            ...
            $this->session->set_flashdata('error','Wallet Balance is low for this Transaction!');
              redirect(ADMINURL.$this->folder.'/'.$this->$pagename);    
            }
    ...
    if( !empty($pdata['contact']) && !empty($pdata['email']) && !empty($pdata['name_on_pancard'])){
        ...
        redirect(ADMINURL.$this->folder.'/non_individual_pan_corr_uploads?id='.md5($lastid) );
        }else{
    ...
    redirect(ADMINURL.$this->folder.'/non_individual_pan_corr');  
    }
