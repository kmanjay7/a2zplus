...
class Wallet_ladger_report_aeps extends CI_Controller{
    var $panename;
    var $folder;
    public function __construct(){
        ...
        $this->load->library('session');   
                sessionunique();  
                $this->pagename = 'Wallet_ladger_report_aeps';
    ...
    public function index(){ 
        ...
        $urluri = ADMINURL.$this->folder.'/'.$this->pagename.'/index/?';
         ...
         if($this->input->get()){
            ...
            $apiurl = APIURL.('webapi/wallet/Wallet_history_aeps'); 
           ...
           //print_r($wtlist); exit;
            ...
            if($wtlist['status']){
                ...
                $data['success_amt'] = fourdecimal($wtlist['t_successfull']);
                $data['total_commission'] = fourdecimal($wtlist['t_comission']);
                $data['total_surcharge'] = fourdecimal($wtlist['t_surcharge']);
                $data['total_tds'] = fourdecimal($wtlist['t_tds']);
            }
        /********* Get recent transactions script end here *********/
 
           if($this->folder=='ad'){
            adview('wallet_ladger_report_aeps',$data );
            }else if($this->folder=='md'){
            mdview('wallet_ladger_report_aeps',$data );
            }else if($this->folder=='bp'){
            bpview('wallet_ladger_report_aeps',$data );
            }  
        }  
