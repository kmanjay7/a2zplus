...
class Md extends CI_Controller{
	...
	public function index($id=false){
            ...
            $data['title'] = 'MD Add';
             ...
             $data['buttonname']      = !empty($id) ? 'Update' : 'Add';
                ...
                $data['admin'] = $this->dg_model->getSingle('users',array('id'=>$parentid),'*');
                $data['state'] = $this->dg_model->getAll('dt_state','statename ASC',array('status'=>'yes'));
                $data['scheme'] = $this->dg_model->getAll('dt_scheme','sch_name ASC',array('status'=>'yes','user_type'=>'MD'));
  ...
  public function view($page=false){ 
        ...
        public function save($id=false){
             ...
             $dpcheck = empty($id) ? (['uniqueid'=>$saveup['uniqueid'],'user_type'=>'MD']):null;
...
/*send update msg to agent end*/
             ...
             if(!empty($update)){
              ...
              redirect(ADMINURL.'bp/md/view'.$parentidx);  
              }else{
              $this->session->set_flashdata('success','Data Submitted Successfully!');
              redirect(ADMINURL.'bp/md/view');      
              }
             }else{
               $this->session->set_flashdata('error','Something Went Wrong!');
                 redirect(ADMINURL.'bp/md');     
             }
