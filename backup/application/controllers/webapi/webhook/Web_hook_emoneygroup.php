...
class Web_hook_emoneygroup extends CI_Controller{
  var $ipaddress;
...
if( ($ip == $this->ipaddress ) && !empty($post)){ 
/************* start code to maintaine order ************************/ 

$Merchantrefno = isset($post['Merchantrefno'])?$post['Merchantrefno']:false;
$Status = isset($post['Status'])?$post['Status']:false;
$TransactionID = isset($post['TransactionID'])?$post['TransactionID']:false;
$apiorderID = isset($post['OrderID'])?$post['OrderID']:false;
