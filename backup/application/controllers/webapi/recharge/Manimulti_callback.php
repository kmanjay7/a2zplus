...
class Manimulti_callback extends CI_Controller{
 ...
 if( ($_SERVER['REQUEST_METHOD'] == 'POST') ){
  ...
  if($rech_data['reqid'] && $rech_data['user_id'] && $rech_data['id'] ){
        ...
        $buffer = $obj->manimulti_transaction_status($Merchantrefno);

    if(isset($buffer['status']) && ($buffer['status']) ){ 
        $Status = $obj->manimulti_status( $buffer['status'] );  
        ...
        if(($action == 'check') && in_array($Status, ['SUCCESS','FAILURE'])){
                ...
                $save_dt['op_transaction_id'] = !empty($buffer['field1'])?$buffer['field1']:'';
                $save_dt['apirefid'] = isset($buffer['apirefid']) && !empty($buffer['apirefid'])?$buffer['apirefid']:''; 
                ...
                if($Status == 'SUCCESS'){
                  ...
                  $save_dt['remark'] = isset($buffer['response'])?$buffer['response']:'Recharge Not Found';
