...
class Rechargemobile extends CI_Controller{
	var $serviceid;
	...
	if( ($_SERVER['REQUEST_METHOD'] == 'POST') ){
				 ...
				 $operator = trim($request['operatorcode']);
				 $amount = trim($request['amount']);
				 $uniqueid = trim($request['uniqueid']);
			...
			$suboperator = isset($request['suboperator'])?trim($request['suboperator']):'simple';


		if(($suboperator == 'special') && in_array($operator,['BSNL','BSNL Special'])){
				$operator = 'BSNL Special';
		}
    ...
    if(!is_null($userdata)){
    	...
    	if(!empty($lastentry)){
    		...
    		if( in_array($lastentry['status'],['PROCESSED','PENDING']) ){
				...
				$response['status'] = false;
				$response['message'] = "Please wait for 5 minutes to initiate new entry with same amount for same mobile number..."; 
				echo json_encode($response);
				exit;
    		}
		...
		if( !empty($userdata) && $mobile && ( strlen($mobile) == 10 ) && $operator && $amount ){
        ...
        if( $wallet >= $amount ){ /*check wallet start here */
        		$orderid = '';	
				$orderid = 'MOB'.date('YmdHis').$userdata['id'].rand(100,999);
				$arr['mobile'] = $mobile; 
				$arr['amount'] = $amount;
				...
				/* find and match api operator codes/simple/special start */ 
				...
				if(empty($op_data_arr)){
					$response['status'] = false;
					$response['message'] = 'No Current Api Set For Mobile Recharge'; 
					echo json_encode($response);
					exit;
				} 
			...
			if(empty($arr['operator'])){
				$response['status'] = false;
				$response['message'] ='No Operator Code Set this API For Mobile Recharge'; 
				echo json_encode($response);
				exit;
			}

			$circleid = $this->c_model->getSingle('operator_circle',array('serviceid'=>$serviceid,'circle'=>$circle),'id');
...
$finalAmount = ( (float)$arr['amount'] - (float)$ag_comi );  
 ...
 if($op_data_arr['currentapiid'] && $op_code_data['operatorid'] && $op_code_data['op_code'] ){
        ...
        $wtsave['remark'] = 'Mobile Recharge'; 
        $wtsave['status'] = 'success'; 
        $wtsave['amount'] = (float)($finalAmount); 
        $wtsave['subject'] = 'mob_rech';
        ...
        /* check wallet for this registration end */
			...
			/********** for multi money ************/
            if( $apiid && $apiid == 1 ){
				$buffer = $obj->mani_prepaid_recharge($arr);  
            }
            /********** for e-money ****************/
            else if($apiid && $apiid == 2){ 
				$buffer = $obj->emoney_recharge($arr);  
            }
            /********** for e-money ****************/
            ...
            /********** for goldpay ****************/
            else if($apiid && $apiid == 13){ 
            	$arr['field1'] = $apioperatorname;
				$buffer = $obj->goldpay_recharge($arr);  
            }
			...
			if(!empty($buffer) && $insertid ){
				...
				$updte = $this->c_model->saveupdate('rech_history',$buffer,null,array('id'=>$insertid)); 
