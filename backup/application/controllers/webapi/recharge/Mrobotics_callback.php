...
class Mrobotics_callback extends CI_Controller{
...
if($rech_data['reqid'] && $rech_data['user_id'] && $rech_data['id'] ){        
        ...
        $buffer = $obj->mrobotics_txn_status($Merchantrefno,$lapuid);



        if(isset($buffer['error']) && empty($buffer['error'])){
            $buffer = $buffer['data'];
        }else if(isset($buffer['error']) && $buffer['error']==1 ){
            $buffer['status'] = 'FAILURE';
            $buffer['tnx_id'] = 'NA';
            $buffer['apirefid'] = 'NA';
        }

        if(isset($buffer['response']) && !empty($buffer['response'])){
            $buffer['response'] = $this->is_json($buffer['response']);
        }
...
if(isset($buffer['status']) && ($buffer['status']) ){ /*buffer init start*/
        ...
        if(($action == 'check') && in_array($Status, ['SUCCESS','FAILURE'])){
                ...
                $save_dt['op_transaction_id'] = !empty($buffer['tnx_id'])?$buffer['tnx_id']:'0';
                $save_dt['apirefid'] = $save_dt['op_transaction_id']; 
                $save_dt['status'] = $Status;
                $save_dt['status_update'] = date('Y-m-d H:i:s');


                if($Status == 'SUCCESS'){
                  $save_dt['remark'] = !empty($buffer['response'])?$buffer['response']:'Your transaction has been SUCCESSED.';
                  $update = $this->c_model->saveupdate('dt_rech_history',$save_dt,null,array('id'=>$tableid ));
                }else if($Status == 'FAILURE'){
                  $save_dt['remark'] = isset($buffer['response']) && !empty($buffer['response'])?$buffer['response']:'INTERNAL_ERROR';
...
public function is_json($string) {
  return  strpos($string, '{') ? 'BAD_REQUEST/INTERNAL_ERROR':'';
}   
