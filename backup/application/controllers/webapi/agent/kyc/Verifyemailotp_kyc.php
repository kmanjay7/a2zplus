...
class Verifyemailotp_kyc extends CI_Controller{
	...
	public function index(){
		...
		$otp = !empty($request['otp']) ? trim($request['otp']) : ''; 
		 ...
		 if(!$tableid){
			$response['status'] = FALSE;
			$response['message'] = 'Table ID is blank!';
			header("Content-Type: application/json");
		    echo json_encode($response);
		    exit;
...
/************fetch user data end script *************/			    
		...
		$kyc_status = 'upload_aadhaar_zip';
		  
	   	$updatearray['kyc_status'] = $kyc_status;
