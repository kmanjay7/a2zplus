...
class Sendmatchotp_kyc extends CI_Controller{
...
/************fetch user data start script *************/
    ...
    $getdata = !empty($getdata) ? $getdata[0] : []; 	 	
...
DigiCash India.';
		...
		if( $getdata['kyc_status'] == '' ){
			$updatearray['kyc_status'] = 'email_verification';
			$kyc_status = 'email_verification';			
		} 
		

		
		if( $getdata['kyc_status'] == 'yes' ){
	   	$updatearray['loginstatus'] = 'yes';
	   	$updatearray['applogin_at'] = date('Y-m-d H:i:s');
		...
		if( in_array($getdata['kyc_status'],['complete','yes']) ){
			...
			$validfrom = strtotime($getdata['fromdate']); 
			$validto = strtotime($getdata['todate']);
			...
			if( $kyc_status != 'onscreening'){ 						 
			$response['status'] = TRUE;
			$response['data'] = $data; 
		    $response['message'] = "OTP verification done successfully!";
			}else if( $kyc_status == 'onscreening' ){
			$response['status'] = TRUE;
			$response['data'] = $data;
		    $response['message'] = "Your KYC is in onscreening!";	
			} 
