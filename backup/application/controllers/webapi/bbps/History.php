...
class History extends CI_Controller{
 ...
 if( ($_SERVER['REQUEST_METHOD'] == 'POST') ){
   ...
   $transaction = !empty($request['transaction'])?$request['transaction']:false;
   $daterange = !empty($request['daterange'])?$request['daterange']:false;
   $filterby = !empty($request['filterby'])?$request['filterby']:"";
   $requestparam = !empty($request['requestparam'])?$request['requestparam']:"";
		...
		$select = 'a.reqid,a.user_id, a.status, a.amount, a.mobileno, a.cust_account_no, a.field2,a.apirefid,a.op_transaction_id, a.operatorname, a.add_date, a.ag_comi, a.ag_tds, a.status_update, dt_operators.image, a.operatorname,a.apirefid,a.status,a.status_update,a.mobileno,a.complaint,a.id,a.duedate,a.billno,a.cons_name'; 
			...
			if( !empty($fetchdata) && !is_null($fetchdata) ){ 
                foreach ($fetchdata as $key => $value) {
						 ...
						 $arr['cust_account_no'] = (string)$value['cust_account_no'];
						 ...
						 $arr['cons_name'] = (string)$value['cons_name'];
						 $arr['billno'] = (string)$value['billno'];
						 $arr['duedate'] = (string)$value['duedate'];

						 $arr['printurl'] = (string) ADMINURL.'ag/bbps_reciept/index?utp='.md5($value['reqid']).'&apptype=app&id='.md5($value['user_id']) ;
