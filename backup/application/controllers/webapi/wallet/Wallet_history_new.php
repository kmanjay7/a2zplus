...
class Wallet_history_new extends CI_Controller{
 ...
 if( ($_SERVER['REQUEST_METHOD'] == 'POST') ){ 
            ...
            $infield = null;
            $invalue = null;
            $keys = "id,credit_debit,subject,referenceid, add_date ,beforeamount ,amount,remark,finalamount,odr,surch,comi,tds,flag";
            $groupby = null;
	...
	if( !empty($getdata) && !is_null($getdata) ){ 
		foreach ($getdata as $key => $value) {
			 ...
			 if($value['flag']){
				$arr['odr_amount'] = (string)twodecimal($value['odr']);
				...
				$arr['close_bal'] = (string) twodecimal($value['finalamount']); 
				 ...
				 $arr['crdr_amount'] = (string) $plusminus.$details['totalamount'];
				 $arr['surcharge'] = (string) $details['surcharge'];
				 $arr['tds'] = (string) $details['tds'];
				 $arr['comission'] = (string) $details['comission'];
				 $arr['close_bal'] = (string) twodecimal($details['closing_bal']);
			 }
...
public function getmainwalt($value){ 
       ...
       $data['totalamount'] = $value['amount'];
          ...
          $dwhere = ''; $dkey = '';
          if( in_array($subject, ['dmt_1','money_transfer']) ){  //money transfer 
