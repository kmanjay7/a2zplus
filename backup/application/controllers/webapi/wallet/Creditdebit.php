...
if( checktoken( WALLETOKEN ) ){ 
			 ...
			 $checkuser = $this->cr_model->countitem_cr($usertable,$wherearray  );
 ...
 /********** check duplicate  wallet entry end***************/
			...
			if( $addby != $userid ){
			$userdata = $this->cr_model->getSingle_cr('dt_users',array('id'=>$userid),'mobileno,ownername,parentid');
			$byuser = 'DigiCash India';  
			if($userdata['parentid'] == $addby && $userdata['parentid']){
				$parntdata = $this->cr_model->getSingle_cr('dt_users',array('id'=>$userdata['parentid']),'mobileno,ownername');
				$byuser = $parntdata['ownername'].'-'.$parntdata['mobileno']; 
			}elseif(!is_numeric($addby)){ $byuser = 'Payment Gatway';  }
$msgbody = "Dear ".strtoupper($userdata['ownername']).", Your wallet has been ".$credit_debit."ed with Rs. ".$amount." and new balance is ".$newtotalamount."; By- ".$byuser.".
Regards,
DigiCash India"; 
